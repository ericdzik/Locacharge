...
import 'dart:convert';
// import 'package:locacharge/core/config/app_config.dart'; // AppConfig n'est plus utilisé ici directement si Mapbox est retiré
import 'package:latlong2/latlong.dart' as latlong; // Correction de l'import pour LatLng
import 'package:locacharge/core/models/eta_result_model.dart';

class MapsService {
  // final String _mapboxAccessToken = AppConfig.mapboxAccessToken; // Commenté car Mapbox n'est plus utilisé pour ETA
  ...
  Future<EtaResult?> getEtaFromOSRM(latlong.LatLng origin, latlong.LatLng destination, String osrmProfile) async {
    // osrmProfile: 'car' (ou 'driving'), 'foot', 'bike'
    String actualProfile = osrmProfile;
    if (osrmProfile == 'driving') actualProfile = 'car'; // OSRM utilise 'car' pour la conduite

    final String coordinates =
        "${origin.longitude},${origin.latitude};${destination.longitude},${destination.latitude}";

    // Utilisation de l'instance publique de démo OSRM
    final String url =
        "http://router.project-osrm.org/route/v1/$actualProfile/$coordinates?overview=false&steps=false";

    print("OSRM API URL: $url");

    try {
      final response = await http.get(Uri.parse(url));
      print("OSRM API Status Code: ${response.statusCode}");
      ...
      if (response.statusCode == 200) {
        ...
        if (data['code'] == 'Ok' && data['routes'] != null && data['routes'].isNotEmpty) {
          ...
          print("OSRM ETA: Duration ${duration}s, Distance ${distance}m");
          return EtaResult(durationSeconds: duration, distanceMeters: distance);
        } else {
          print("Erreur de l'API OSRM: ${data['code']} - ${data['message'] ?? 'Aucune route trouvée ou erreur inconnue'}");
          return null;
        }
      } else {
        print("Erreur HTTP lors de l'appel à OSRM: ${response.statusCode} - ${response.body}");
        return null;
      }
    } catch (e) {
      print("Exception lors de l'appel à OSRM: $e");
      return null;
    }
  }

  // Ancienne méthode Google ETA, peut être conservée pour référence ou supprimée si non utilisée.
  // Future<String> getEtaFromGoogle(latlong.LatLng origin, latlong.LatLng destination) async { // Note: LatLng type changed
